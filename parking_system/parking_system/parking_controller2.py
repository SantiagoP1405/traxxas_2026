...
class ParkingController2(Node):

    def __init__(self):
        super().__init__('parking_controller2_qcar')
        ...
        # --- FLAG PARA LA MANIOBRA DE CORRECCIÓN ---
        ...
        self.get_logger().info(
            'Parking Controller 2 QCar Iniciado (Con Reflejo de Corrección Lateral y Volante)'
        )
    ...
    def imu_cb(self, msg):
        ...
        if self.yaw_initial is None:
            ...
            self.get_logger().info(
                f'YAW inicial capturado: {math.degrees(raw_yaw):.2f}°'
            )
    ...
    def loop(self):
        ...
        # --- SI CUALQUIER FRENO PERMANENTE ESTÁ ACTIVO ---
        if (
            self.freno_emergencia
            or self.freno_stop_dist
            or self.freno_final_dist
        ):

            if self.freno_emergencia:
                self.get_logger().error(
                    f'Vehículo bloqueado: FRENO EMERGENCIA ({self.ub:.2f}m)'
                )

            elif self.freno_stop_dist:
                self.get_logger().warn(
                    f'Vehículo bloqueado: STOP_DIST alcanzado ({self.ub:.2f}m)'
                )

            elif self.freno_final_dist:
                self.get_logger().warn(
                    f'Vehículo bloqueado: FINAL_DIST alcanzado ({self.ub:.2f}m)'
                )

            self._publicar(0.0, 0.0)
            return

        # Capturar Yaw al entrar en ENDEREZAR
        ...
        if self.state == 'BUSCAR':
            ...
            self.get_logger().info(
                f'# debug yaw | yaw_buscar actualizado: '
                f'{math.degrees(self.yaw_buscar):.2f}°'
            )
            ...
            self.get_logger().info(
                f'# debug yaw | yaw={math.degrees(self.yaw):.2f}° '
                f'target={math.degrees(yaw_target):.2f}° '
                f'err={math.degrees(yaw_err):.2f}°'
                f'lat_err={lat_err:.3f}m '
                f'ul={self.ul:.2f} ur={self.ur:.2f}'
            )
            ...
            if self.ub <= STOP_DIST:
                ...
                self.get_logger().warn(
                    f'STOP_DIST alcanzado: {self.ub:.2f}m — freno permanente'
                )
            ...
            if self.ub > FINAL_DIST + FINAL_TOL:
                ...
                self.get_logger().warn(
                    f'FINAL_DIST alcanzado: {self.ub:.2f}m — freno permanente'
                )

                self.freno_final_dist = True

        # ==========================================================
        # SISTEMA DE SUPERVIVENCIA LATERAL
        # ==========================================================
        ...
        if self.state in ['ENTRAR_DIAGONAL', 'ENDEREZAR'] and speed_cmd < 0.0:
            ...
            if now > self.correcting_until_time:
                ...
                if (
                    self.parking_side == 'RIGHT'
                    and self.ul <= LATERAL_DANGER_DIST
                ):

                    self.get_logger().warn(
                        f'¡Peligro Izq ({self.ul:.2f}m)! '
                        'Avanzando y girando IZQ...'
                    )
                    ...
                    and self.ur <= LATERAL_DANGER_DIST
                ):

                    self.get_logger().warn(
                        f'¡Peligro Der ({self.ur:.2f}m)! '
                        'Avanzando y girando DER...'
                    )
        ...
        # ==========================================================
        # FRENO DE EMERGENCIA TRASERO
        # ==========================================================

        if speed_cmd < 0.0 and self.ub <= EMERGENCY_STOP_DIST:

            self.get_logger().error(
                f'¡FRENO EMERGENCIA TRASERO PERMANENTE! '
                f'Objeto a {self.ub:.2f}m'
            )
    ...
    def _publicar(self, speed_cmd, steer_cmd):
        ...
        hardware_steer = self.clamp(
            logical_steer + STEERING_OFFSET,
            MAX_STEER_LIMIT
        )
