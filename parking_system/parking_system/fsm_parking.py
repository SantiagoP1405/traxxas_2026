...
class ParkingFSM(Node):
    def __init__(self):
        ...
        self.confirm_start_time = None  #nuevo
    ...
    def perception_cb(self, msg):
        ...
        # 2. Detectar espacio con confirmación
        if fc and rf and lf:
            # empieza a confirmar si no ha empezado
            if self.confirm_start_time is None:
                self.confirm_start_time = time.time()
                self.get_logger().info('Posible espacio, confirmando...')

            # si ya lleva CONFIRM_TIME viéndolo, arranca DETECT_EXTRA_TIME
            ...
            # si deja de verlo, resetea la confirmación
