#!/usr/bin/env python3

import rclpy
from rclpy.node import Node
from sensor_msgs.msg import Imu
from std_msgs.msg import String
from geometry_msgs.msg import Vector3Stamped
import math
import serial
import time

# --- CONFIGURACION DE HARDWARE ---
SERIAL_PORT = '/dev/ttyUSB0' 
BAUD_RATE = 115200

# --- LIMITES DE SEGURIDAD FÍSICA QCAR ---
MAX_SPEED_LIMIT = 0.12  
MAX_STEER_LIMIT = 0.5   

# --- CALIBRACIÓN MECÁNICA (TRIM) ---
STEERING_OFFSET = -0.0812

# --- VELOCIDADES (LENTO PERO SEGURO) ---
SEARCH_SPEED  = 0.08
PARKING_SPEED = -0.08

# --- CONSTANTES DE TU SIMULACION ---
KP_YAW = 0.5 
KP_LAT = 15.0 
MAX_ANG_PID = 0.35

STOP_DIST = 0.08   # Metros
ALIGN_TOL = 0.04
CENTER_TOL = 0.10

# --- AJUSTE FINO FINAL ---
FINAL_DIST = 0.05
FINAL_TOL  = 0.01
FINAL_SPEED = -0.05

# --- PARÁMETRO DE SEGURIDAD CRÍTICA ---
EMERGENCY_STOP_DIST = 0.07

# --- CORRECCIÓN DIAGONAL ---
DIAG_CORRECTION_THRESH = 0.05
DIAG_CORRECTION_GAIN   = 0.08

# --- REFERENCIA YAW ---
YAW_REF = math.pi / 2


class ParkingController(Node):
    def __init__(self):
        super().__init__('parking_controller_qcar')

        self.state = 'BUSCAR'
        self.parking_side = 'RIGHT'   
        self.yaw = 0.0
        self.yaw_initial = None
        
        self.prev_state = None 
        self.yaw_target = None

        # --- FLAGS DE FRENO PERMANENTE ---
        self.freno_emergencia = False
        self.freno_stop_dist  = False
        self.freno_final_dist = False

        # Sensores (en METROS)
        self.ul = 1.0
        self.ub = 1.0
        self.ur = 1.0

        # --- CONEXION ARDUINO ---
        try:
            self.ser = serial.Serial(SERIAL_PORT, BAUD_RATE, timeout=0.1)
            self.get_logger().info(f'--- CONECTADO A ARDUINO ({SERIAL_PORT}) ---')
            time.sleep(2)
            self.ser.reset_input_buffer()
        except Exception as e:
            self.get_logger().error(f'ERROR ARDUINO: {e}')
            self.ser = None

        # --- SUSCRIPCIONES QCAR ---
        self.create_subscription(String, '/parking/state', self.state_cb, 10)
        self.create_subscription(Imu, '/imu/data', self.imu_cb, 10)

        # --- PUBLICADORES QCAR ---
        self.cmd_pub = self.create_publisher(Vector3Stamped, '/qcar/user_command', 10)
        self.state_pub = self.create_publisher(String, '/parking/state_feedback', 10)

        self.timer = self.create_timer(0.05, self.loop)
        self.get_logger().info('Parking Controller QCar Iniciado (Frenos Permanentes + Calibración)')

    def state_cb(self, msg):
        if ':' in msg.data:
            self.state, self.parking_side = msg.data.split(':')
        else:
            self.state = msg.data

    def imu_cb(self, msg):
        q = msg.orientation
        # Conversión manual quaternion → yaw
        raw_yaw = math.atan2(
            2.0 * (q.w * q.z + q.x * q.y),
            1.0 - 2.0 * (q.y * q.y + q.z * q.z)
        )
        # Capturar yaw inicial para que siempre arranque en 0°
        if self.yaw_initial is None:
            self.yaw_initial = raw_yaw
            self.get_logger().info(f'YAW inicial capturado: {math.degrees(raw_yaw):.2f}°')
        self.yaw = raw_yaw - self.yaw_initial

    def filter_range(self, value, prev):
        if value <= 0.05 or value > 5.0:
            return prev
        return 0.3 * prev + 0.7 * value

    def read_ultrasonics_fast(self):
        if self.ser and self.ser.in_waiting > 0:
            try:
                raw_data = self.ser.read_all().decode('utf-8', errors='ignore')
                lines = raw_data.split('\n')
                for line in reversed(lines):
                    if ',' in line and len(line.split(',')) == 3:
                        parts = line.strip().split(',')
                        try:
                            new_ul = float(parts[0]) / 100.0
                            new_ub = float(parts[1]) / 100.0
                            new_ur = float(parts[2]) / 100.0

                            self.ul = self.filter_range(new_ul, self.ul)
                            self.ur = self.filter_range(new_ur, self.ur)

                            # ub SIN filtro para seguridad crítica
                            if 0.05 < new_ub < 2.0:
                                self.ub = new_ub

                            return
                        except ValueError:
                            continue
            except Exception:
                pass

    def clamp(self, value, limit):
        return max(-limit, min(value, limit))

    def loop(self):
        self.read_ultrasonics_fast()

        # --- SI CUALQUIER FRENO PERMANENTE ESTÁ ACTIVO, PARAR Y NO HACER NADA MÁS ---
        if self.freno_emergencia or self.freno_stop_dist or self.freno_final_dist:
            if self.freno_emergencia:
                self.get_logger().error(f'Vehículo bloqueado: FRENO EMERGENCIA ({self.ub:.2f}m)')
            elif self.freno_stop_dist:
                self.get_logger().warn(f'Vehículo bloqueado: STOP_DIST alcanzado ({self.ub:.2f}m)')
            elif self.freno_final_dist:
                self.get_logger().warn(f'Vehículo bloqueado: FINAL_DIST alcanzado ({self.ub:.2f}m)')

            self._publicar(0.0, 0.0)
            return

        # Detectar cambio de estado para capturar Yaw
        if self.state == 'ENDEREZAR' and self.prev_state != 'ENDEREZAR':
            self.yaw_target = YAW_REF if self.parking_side == 'RIGHT' else -YAW_REF
            self.get_logger().warn(f'Yaw target capturado: {self.yaw_target:.3f}')

        self.prev_state = self.state

        speed_cmd = 0.0
        steer_cmd = 0.0

        if self.state == 'BUSCAR':
            speed_cmd = SEARCH_SPEED
            steer_cmd = 0.0

        elif self.state == 'ESPERAR':
            speed_cmd = 0.0
            steer_cmd = 0.0

        elif self.state == 'ENTRAR_DIAGONAL':
            speed_cmd = PARKING_SPEED

            base_steer = 0.5 if self.parking_side == 'RIGHT' else -0.5

            lat_diff = self.ul - self.ur
            if lat_diff > DIAG_CORRECTION_THRESH:
                correction = +DIAG_CORRECTION_GAIN #antes - diag
            elif lat_diff < -DIAG_CORRECTION_THRESH:
                correction = -DIAG_CORRECTION_GAIN #antes + diag
            else:
                correction = 0.0

            steer_cmd = self.clamp(base_steer + correction, MAX_STEER_LIMIT)

            self.get_logger().info(
                f'DIAGONAL | ul={self.ul:.2f} ur={self.ur:.2f} '
                f'diff={lat_diff:.3f} corr={correction:.3f} steer={steer_cmd:.3f}'
            )

        elif self.state == 'ENDEREZAR':
            speed_cmd = PARKING_SPEED

            yaw_target = self.yaw_target if self.yaw_target else self.yaw

            yaw_err = math.atan2(
                math.sin(yaw_target - self.yaw),
                math.cos(yaw_target - self.yaw)
            )

            lat_err = self.ul - self.ur
            ang = KP_YAW * yaw_err - KP_LAT * lat_err  # antes era +KP_LAT
            steer_cmd = self.clamp(ang, MAX_ANG_PID)

            if abs(yaw_err) < ALIGN_TOL and abs(lat_err) < CENTER_TOL:
                self.state_pub.publish(String(data='ENDEREZADO'))

        elif self.state == 'ESTACIONANDO':
            speed_cmd = PARKING_SPEED

            if self.ub <= STOP_DIST:
                self.get_logger().warn(f'STOP_DIST alcanzado: {self.ub:.2f}m — freno permanente activado')
                self.freno_stop_dist = True
                self.state_pub.publish(String(data='DIST_OK'))

        elif self.state == 'STOP':
            if self.ub > FINAL_DIST + FINAL_TOL:
                speed_cmd = FINAL_SPEED
                steer_cmd = 0.0
            else:
                self.get_logger().warn(f'FINAL_DIST alcanzado: {self.ub:.2f}m — freno permanente activado')
                self.freno_final_dist = True

        # --- FRENO DE EMERGENCIA PERMANENTE ---
        if speed_cmd < 0.0 and self.ub <= EMERGENCY_STOP_DIST:
            self.get_logger().error(f'¡FRENO EMERGENCIA PERMANENTE! Objeto a {self.ub:.2f}m')
            self.freno_emergencia = True
            speed_cmd = 0.0
            steer_cmd = 0.0

        self._publicar(speed_cmd, steer_cmd)

    def _publicar(self, speed_cmd, steer_cmd):
        final_speed = self.clamp(speed_cmd, MAX_SPEED_LIMIT)
        logical_steer = self.clamp(-steer_cmd, MAX_STEER_LIMIT)
        hardware_steer = self.clamp(logical_steer + STEERING_OFFSET, MAX_STEER_LIMIT)

        msg = Vector3Stamped()
        msg.header.stamp = self.get_clock().now().to_msg()
        msg.header.frame_id = 'command_input'
        msg.vector.x = float(final_speed)
        msg.vector.y = float(hardware_steer)
        msg.vector.z = 0.0

        self.cmd_pub.publish(msg)


def main():
    rclpy.init()
    node = ParkingController()
    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        pass
    finally:
        stop = Vector3Stamped()
        stop.header.stamp = node.get_clock().now().to_msg()
        node.cmd_pub.publish(stop)
        node.destroy_node()
        rclpy.shutdown()

if __name__ == '__main__':
    main()
